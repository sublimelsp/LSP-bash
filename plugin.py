...
PACKAGE_NAME = "LSP-bash"
SETTINGS_FILENAME = "LSP-bash.sublime-settings"
...
class LspBashPlugin(LanguageHandler):
    ...
    def config(self) -> ClientConfig:
        ...
        default_configuration = {
            "enabled": True,
            "command": ["node", server.binary_path, "start"],
        }

        default_configuration.update(configuration)

        return read_client_config("lsp-bash", default_configuration)
    ...
    def on_start(self, window) -> bool:
        if not is_node_installed():
            sublime.status_message("Please install Node.js for the bash Language Server to work.")
            return False
        return server.ready
